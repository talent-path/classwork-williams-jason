...
// let heroes = ["blank","anti-mage","axe","bane","bloodseeker","crystal maiden",
...
const getProfileInfo = function () {
    ...
    profileIcone.style.border = "50px";
    $("#recentMatches").text("");
}
const getRecentGames = function () {
    ...
    $("#recentMatchTop").text("Your 10 most recent games");
    $("#recentMatches").text("");
    recentMatchesContainer.style.visibility = "visible";

    $.get(

        `https://api.opendota.com/api/players/${playerIdForMatch}/matches`,
        function (data, textStatus, jqXHR) {
            for (let i = 0; i < 10; i++) {

                let minutes = data[i].duration / 60
                let secondsHelper = parseInt(minutes)
                let seconds = (minutes - secondsHelper) * 60;
                minutes = Math.trunc(minutes)
                seconds = Math.trunc(seconds)

                for (let j = 0; j < 120; j++) {
                    if (data[i].hero_id === heroPool[j].id) {
                        heroName = heroPool[j].localized_name;
                        break;
                    }
                }
                ...
                $("#recentMatches").append(`Match duration: ` + minutes + ` minutes and ` + seconds + ` seconds `)
                $("#recentMatches").append(`Hero Id : ` + heroName + ` `)
                ...
                if (data[i].deaths == 0) {
                    $("#recentMatches").append(`K/D: ` + 1.00)
                } else {
                    $("#recentMatches").append(`K/D: ` + (data[i].kills / data[i].deaths).toFixed(2))
                }
                if ((data[i].radiant_win && data[i].player_slot <= 127) || (!data[i].radiant_win && data[i].player_slot >= 128)) {
                    $("#recentMatches").append(" Win ")
                    winCount++;
                } else {
                    $("#recentMatches").append(" Loss ")
                    lossCount++;
                }
                $("#recentMatchTop").text("Win/Loss Ratio: " + winCount + "/" + lossCount);
